...
type PerpsOrderController struct {
	e           exchange.Exchange
	client      ws.Client
	stopC       chan struct{}
...
func NewPerpsOrderController(e exchange.Exchange, url, localIP, apiKey string, secretKey ed25519.PrivateKey, heartBeat int) (*PerpsOrderController, error) {
	oc := &PerpsOrderController{
		e:     e,
...
func (oc *PerpsOrderController) Close() {
	oc.rLock.Lock()
...
// PlaceOrder 下单
func (oc *PerpsOrderController) PlaceOrder(newOrder exchange.NewOrder) (*exchange.OrderDetail, error) {
	var (
...
func (oc *PerpsOrderController) CancelOrder(clientOrderID string) error {
	var (
...
func (oc *PerpsOrderController) CancelAllOrder(symbol exchange.SymbolName) error {
	var (
		resC = make(chan *fastjson.Value)
		id   = utils.GetUUID()
	)
...
func (oc *PerpsOrderController) ReplaceOrder(cancelOrigClientOrderId string, newOrder exchange.NewOrder) (*exchange.OrderDetail, error) {
	return nil, nil
}

func (oc *PerpsOrderController) callback(msg []byte) {
...
func (oc *PerpsOrderController) login(apiKey string, secretKey ed25519.PrivateKey) error {
	params := map[string]string{
		"apiKey":    apiKey,
		"timestamp": strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10),
	}
...
func (oc *PerpsOrderController) sendMessage(msg []byte) error {
	oc.rLock.RLock()
...
var _ exchange.OrderController = (*PerpsOrderController)(nil)
