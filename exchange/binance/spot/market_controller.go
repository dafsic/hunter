...
type SpotMarketController struct {
	e           exchange.Exchange
	client      ws.Client
	rLock       sync.RWMutex
	stopC       chan struct{}
...
func NewSpotMarketController(e exchange.Exchange, url, localIP string, heartBeat int) (*SpotMarketController, error) {
	mc := &SpotMarketController{
		e:           e,
...
func (mc *SpotMarketController) Close() {
	mc.rLock.Lock()
...
func (mc *SpotMarketController) BookTickerChannel() <-chan *exchange.BookTicker {
	return mc.bookTickerC
}

func (mc *SpotMarketController) callback(msg []byte) {
	// {
...
var _ exchange.MarketController = (*SpotMarketController)(nil)
